...
const CardsBusca = async () => {
  ...
  let returFilter = jsonLinkCardDados.filter(item => {
      if(item.nome.charAt(0) == FirstLetra) return item
  })

  let createCard = returFilter.filter(item => {
    if(inputBusca.value == item.nome) return item
  })

  createCard.map(async({uri}) => {
    let uriDeputadosUri = await fetch(uri)
...
function deputados() {
  fetch(urlLink)
    .then(async resultDeputados => {
      ...
      let deputadoInfo = await resultDeputados.json();
      let deputadosDados = deputadoInfo.dados
